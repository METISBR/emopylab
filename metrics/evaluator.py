...
from metrics.indicators import HV as _NativeHV, R2 as _NativeR2
...
def r2_indicator(
    ...
    """Calculate R2 indicator using native tensor vectorization."""
    return float(_NativeR2(pf=PF, ref_dirs=W, n_partitions=n_partitions).do(F))
...
class MetricEvaluator:
    ...
    def evaluate(cls,
        ...
        if normalized_name in {"R2", "R2INDICATOR"}:
            W = ctx.get("ref_dirs") or ctx.get("weights") or ctx.get("W")
            p_part = ctx.get("r2_partitions")
            return r2_indicator(
                F,
                PF=pf_arr if (pf_arr is not None and pf_arr.size > 0) else None,
                W=W,
                n_partitions=p_part,
            )
