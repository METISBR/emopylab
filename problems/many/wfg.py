...
"""
WFG benchmark family converted for local use in EmoPyLab.

Reference
---------
S. Huband, P. Hingston, L. Barone, and L. While. A review of
multiobjective test problems and a scalable test problem toolkit.
IEEE Transactions on Evolutionary Computation, 2006, 10(5): 477-506.
"""

import numpy as np

from core.problem import Problem
from util.ref_dirs import generic_sphere, get_ref_dirs
from util.functions import load_function
from util.misc import powerset, default_random_state


class WFG(Problem):

    def __init__(self, n_var=None, n_obj=3, k=None, l=None, **kwargs):
...
class WFG3(WFG):
    ...
    def __init__(self, n_var=None, n_obj=3, k=None, **kwargs):
...
_WFG_CPU = ["WFG1", "WFG2", "WFG3", "WFG4", "WFG5", "WFG6", "WFG7", "WFG8", "WFG9"]
for _name in _WFG_CPU:
    _base = globals()[_name]
    globals()[f"{_name}_JAX"] = type(f"{_name}_JAX", (_base,), {})

__all__ = _WFG_CPU + [f"{n}_JAX" for n in _WFG_CPU]
